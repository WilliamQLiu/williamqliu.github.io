...
*  [Views](#views)
    ...
    -  [render()](#render)
*  [Class Based Views and Generic Class Based Views](#cbv)
    -  [CBV Workflow](#cbvflow)
    -  [CBV Mixins](#cbvmixins)
    -  [CBV Debugging](#cbvdebug)
*  [URLs](#urls)
...
##<a id="modelmanagers">Model Managers</a>
...
####<a id="cbvdebug">Debugging GCBVs</a>
...
####<a id="urlregexpatterns">URL Regex Patterns</a>
...
With a lot of apps, we'll have multiple __urls.py__ files.  What happens is that when you use `reverse()` template function, we might get the wrong URL if the URL-pattern matches the name in another app.
...
Template tags can have for-loops and if, elif, else statements.  Variables can also be passed from the view to the template.  If any variables are invalid, they are seen as __None__.
...
If we want to move away from the Django HTML Form Template and into Python code, we can do that with Django forms.  When we look at Django's __forms__, we need to be aware if the form is __bound__ (data associated with it) or __unbound form__ (no data associated with it).  You can check if a form is bound by checking the attribute `.is_bound`
...
####<a id="sessiondatastorage">How Django stores Sessions</a>
