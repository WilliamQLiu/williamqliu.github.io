...
With a lot of apps, we'll have multiple __urls.py__ files.  We can use `include(mysmallerapp.urls)` to add the urls in the file `mysmallerapp.urls` into the global namespace.

If you  specify the arguments __namespace__ and __app_name__, we can determine the current app and call the app_name based on the namespace.  For example:
...
You can access Session data outside of a View with SessionStore.
