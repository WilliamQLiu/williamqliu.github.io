...
If I want to add, delete or move files, I type in `m` to open up a menu for that.
...
* `:split` to split windows (two horizontal windows), also `:vsplit` to split to vertical windows (can also `Ctrl` + `w` + `v` for a new split window)
