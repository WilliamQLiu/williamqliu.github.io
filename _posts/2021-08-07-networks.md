...
## Tools (`ipcalc` and `dig`)

Use `ipcalc` (`brew install ipcalc`)
...
Use dig to see the subnets, CIDR blocks, etc.
```
brew install bind
```
Run in:
```
dig redash.zp-int.com

;; Query time: 142 msec
;; SERVER: 127.0.1.3#53(127.0.1.3)
;; WHEN: Fri Jan 31 13:39:09 MST 2025
;; MSG SIZE  rcvd: 193
```

And you'll be able to see the subnets (e.g. `internal-data-prod-eks-data-lb....us-west-2.elb.amazonaws.com 20 A X.X.X.X`)

Will show the Route of a CIDR (e.g. `10.40.0.0/16` with a Destination and a Target

Can run a debug shell and install cli tools

```
kubectl run my-debug-shell --rm -it --image ubuntu -- /bin/bash
```
